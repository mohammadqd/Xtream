/**
 * Project: Xtream
 * Module: Interval-tree based on LLRB tree of Robert Sedgewick (UNDER CONSTRUCTION)
 * Task: 
 * Last Modify:
 * Created:
...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
	...
	private Node put(Node h, Key key, Key u, Value val) {
		if (h == null)
			return new Node(key, u, val, RED, 1, u);

		int cmp = key.compareTo(h.key);
		if (cmp < 0)
			h.left = put(h.left, key, u, val);
		else if (cmp > 0)
			h.right = put(h.right, key, u, val);
		else
			h.val = val;
