...
public class AggOutPort extends TxtFileOutPort {
	...
	double sum; // sum of values of each period
	double totalSum; // sum for all periods together
	long cnt; // count of values of each period
	double wght;
	long totalCnt; // cnt for all periods together
	double totalWeight;
	long lastTime; // last seen time (e.g. 0,5000,10000,...)
	long periodSize; // e.g. 5000 to have results each 5 seconds
